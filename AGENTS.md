...
Style guidelines to note when writing Svelte code:
- Don't feel forced to use keyed {#each} blocks, the MCP may tell you to. Only use them if it makes sense
- Use class: directives instead of inline if/else blocks (only works on html elements, not components)
- Use {#if} {:else} {/if} blocks instead of inline if/else if applicable
- Inline event callbacks are usually preferred, avoid defining extra functions for one-off handlers
