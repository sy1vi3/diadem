...
export async function updateVisibleWeatherCells() {
	...
	if (missing.length) {
		const encoded = encodeRequestBody({ cellIds: missing });
		...
		if (response.ok) {
			const rows = await parseResponse<WeatherCellRow[]>(response);
			for (const row of rows) cellWeatherCache.set(String(row.id), row);
		}
	}

	if (!weatherDisplayActive || requestVersion !== weatherRequestVersion || getMap() !== map) return;
