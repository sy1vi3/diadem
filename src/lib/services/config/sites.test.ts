...
const base = {
	...
	mapStyles: [],
	uiconSets: [],
	defaultFilters: { pokemon: { enabled: false } }
} as unknown as ClientConfig;

it("merges presentation fields, replaces filter presets, and does not change base config", () => {
	const before = structuredClone(base);
	const configs = createSiteConfigs(base, [
		{
			origin: "https://north.example/",
			client: {
				general: { mapName: "North", defaultLat: 20, showAccessMaps: false },
				discord: { serverId: "north" },
				defaultFilters: { pokemon: { enabled: true } }
			}
		}
	]);
	...
	expect(north.general).toMatchObject({
		...
		showAccessMaps: false
	});
	expect(north.discord).toEqual({ serverId: "north", serverLink: "base-link" });
	expect(north.defaultFilters?.pokemon?.enabled).toBe(true);
